...
const changeScore = (num: number) => {
  if (blockProps.block.value === num.toString()) {
    blockProps.block.value = "";
    scoreNum.value = 0;
    return;
  }
  blockProps.block.value = num.toString();
  scoreNum.value = num;
  if (num > 0) {
...
function toggleChildren(childrens: any, index: any) {
  emit("toggleChildren", childrens, index);
}
function countProgress() {
  emit("countProgress");
}
function changeForm(childrens: any, index: any) {
  ...
  >
    ...
    <div class="flex">
      <StarIcon
        ...
        @click="changeScore(1)"
        :class="{
          'text-cyan-500': scoreNum >= 1,
          'text-gray-200': scoreNum < 1,
        }"
      ></StarIcon>
      <StarIcon
        class="w-12"
        @click="changeScore(2)"
        :class="{
          'text-cyan-500': scoreNum >= 2,
          'text-gray-200': scoreNum < 2,
        }"
      ></StarIcon>
      <StarIcon
        class="w-12"
        @click="changeScore(3)"
        :class="{
          'text-cyan-500': scoreNum >= 3,
          'text-gray-200': scoreNum < 3,
        }"
      ></StarIcon>
      <StarIcon
        class="w-12"
        @click="changeScore(4)"
        :class="{
          'text-cyan-500': scoreNum >= 4,
          'text-gray-200': scoreNum < 4,
        }"
      ></StarIcon>
      <StarIcon
        class="w-12"
        @click="changeScore(5)"
        :class="{
          'text-cyan-500': scoreNum >= 5,
          'text-gray-200': scoreNum < 5,
        }"
      ></StarIcon>
    </div>
    <div
